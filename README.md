...
<p align="center">
  ...
  <a href="https://sebastienrousseau.com/java-config/"><img src="https://img.shields.io/badge/docs-sebastienrousseau.com-brightgreen.svg?style=for-the-badge&logo=github" alt="Documentation" /></a>
...
- [Enterprise Checkstyle, SpotBugs & PMD Rules](#enterprise-checkstyle-spotbugs--pmd-rules) — before and after rule comparison
...
Install `@sebastienrousseau/java-config` as a development dependency:

```bash
# npm
npm install --save-dev @sebastienrousseau/java-config

# pnpm
pnpm add -D @sebastienrousseau/java-config

# yarn
yarn add -D @sebastienrousseau/java-config

# bun
bun add -d @sebastienrousseau/java-config
```

### Build from source

```bash
git clone https://github.com/sebastienrousseau/java-config.git
cd java-config
make            # check + test
```
...
- **Target environment**: Optimized for Java 21+ / Checkstyle.
...
```bash
cp node_modules/@sebastienrousseau/java-config/checkstyle.xml ./checkstyle.xml
cp node_modules/@sebastienrousseau/java-config/spotbugs-exclude.xml ./spotbugs-exclude.xml
cp node_modules/@sebastienrousseau/java-config/pmd-ruleset.xml ./pmd-ruleset.xml
```

### Or point the tool at the installed file

```bash
java -jar checkstyle.jar -c node_modules/@sebastienrousseau/java-config/checkstyle.xml src
```
...
`@sebastienrousseau/java-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/java-config` solves this with three deliberate engineering choices:
...
## Enterprise Checkstyle, SpotBugs & PMD Rules

Guarantees enterprise-grade code cleanliness, memory safety, concurrency verification, and modern Java 21 syntax enforcement.
...
### After (@sebastienrousseau/java-config Enforced)
...
When maintaining ancient Java 6/7/8 codebases unable to compile modern record patterns or module descriptors.
