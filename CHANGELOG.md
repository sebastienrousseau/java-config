...
- Added a `native-validate` CI job that installs the real toolchain (checkstyle, pmd, spotbugs) and validates the shipped presets.
- Corrected the OpenSSF Best Practices badge, which pointed at another project.
## [0.0.1] - 2026-09-09

### Added

- Initial release of `@sebastienrousseau/java-config` conforming to REPO-STANDARD.
- Standardized Java configuration presets: `checkstyle.xml`, `spotbugs-exclude.xml`, `pmd-ruleset.xml`.
