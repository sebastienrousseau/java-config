...
# POSIX-compatible Makefile for java-config.
...
lint:
	npm install --no-save --no-audit --no-fund eslint@10.10.0 prettier@3.6.2 @sebastienrousseau/eslint-config @sebastienrousseau/prettier-config
	npx eslint --no-config-lookup --config node_modules/@sebastienrousseau/eslint-config/index.cjs --max-warnings 0 $(LINT_PATHS)
