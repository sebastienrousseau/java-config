...
%global npm_bare      java-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-java-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable Java configuration providing standardized Checkstyle, SpotBugs, and PMD rules adhering to modern 2026 standards

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/java-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/java-config is a shareable configuration preset with no runtime
...
install -pm 0644 checkstyle.xml %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 spotbugs-exclude.xml %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 pmd-ruleset.xml %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
