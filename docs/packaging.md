...
# Packaging @sebastienrousseau/java-config
...
-  `checkstyle.xml`, `spotbugs-exclude.xml`, `pmd-ruleset.xml` — the tool-native preset file(s)
...
## A note on the distribution channel

This package ships configuration for a toolchain that does not read npm. The
presets are consumed by copying them into a project or by pointing the tool at
them explicitly — see [the README](../README.md#quick-start) for the exact
invocation, and [ADR 0002](./adr/0002-distribution-channel.md) for why npm is
nonetheless the chosen channel.

If you are packaging for a native ecosystem (crates.io, PyPI, RubyGems, a distro
package), the useful artefacts are the preset files themselves; they carry no
JavaScript and have no dependencies.
...
| :----- | :--- | :---------- |
| Debian | [`pkg/deb/`](../pkg/deb) | `/usr/share/nodejs/@sebastienrousseau/java-config/` |
| RPM | [`pkg/rpm/`](../pkg/rpm) | `/usr/lib/node_modules/@sebastienrousseau/java-config/` |
| Arch | [`pkg/aur/PKGBUILD`](../pkg/aur/PKGBUILD) | `/usr/lib/node_modules/@sebastienrousseau/java-config/` |
| Homebrew | [`pkg/brew/java-config.rb`](../pkg/brew/java-config.rb) | formula cellar |
| Nix | [`pkg/nix/flake.nix`](../pkg/nix/flake.nix) | `$out/lib/node_modules/@sebastienrousseau/java-config/` |
